...
const h3NodeSringifier: NodeStringifier<h3Content> = {
  toMarkdown: function(node) {
    return `### ${node.content}`;
  },
  canStringifyNode: function(node) {
    return node.type === h3NodeType;
  }
};
...
function getListStringifier(
  getListPlaceholder: (contentItem: MarkdownNode<any>, index: number) => string
) {
  return function(
    ...
    return node.content
      .map((contentItem, index) => {
        if (orderedListNodeStringifier.canStringifyNode(contentItem)) {
          ...
          return `${tabs}${getListPlaceholder(
            contentItem,
            index
          )} ${toMarkdownString(contentItem)}`;
        }
      })
      .join("\n");
  };
